...
class TempOverlay(
    private val ctx: Context, private val wm: WindowManager, private val tmp: TempHelper
) {
    ...
    fun create() {
        v = LayoutInflater.from(ctx).inflate(R.layout.overlay_temp,null)
        tv = v!!.findViewById(R.id.tv_temp)
        ...
        ).apply { width=WindowManager.LayoutParams.WRAP_CONTENT;height=WindowManager.LayoutParams.WRAP_CONTENT;gravity=Gravity.TOP or Gravity.START;x=20;y=460 }
    ...
    fun update() {
        val t = tmp.tempC
        tv?.text = "TEMP: ${t}°C"
        tv?.setTextColor(when{ t>70 -> Color.RED; t>55 -> Color.parseColor("#FF9900") else Color.parseColor("#00FF99") })
    }
