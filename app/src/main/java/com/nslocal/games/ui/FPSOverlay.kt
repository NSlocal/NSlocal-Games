...
class FPSOverlay(private val ctx: Context, private val onFpsUpdate: (Int) -> Unit) : TextView(ctx), View.OnTouchListener {
    private var fps = 60; private var lastTime = System.nanoTime(); private var frames = 0
    ...
    ).apply { width = -2; height = -2; gravity = Gravity.TOP or Gravity.START; x = 20; y = 20 }

    init {
        setTextColor(Color.parseColor("#22C55E")); textSize = 14f; setBackgroundColor(Color.parseColor("#CC000000")); setPadding(12,6,12,6); setOnTouchListener(this)
        Handler(Looper.getMainLooper()).post(object : Runnable {
            override fun run() {
                val now = System.nanoTime(); frames++
                if (now - lastTime >= 1_000_000_000) {
                    fps = (frames * 1_000_000_000f / (now - lastTime)).toInt()
                    text = "🎮 FPS: $fps"; onFpsUpdate(fps); frames = 0; lastTime = now
                }
