...
class FPSOverlay(private val ctx: Context) : TextView(ctx), View.OnTouchListener {
    ...
    init {
        setTextColor(Color.parseColor("#00E676")); textSize = 14f; setBackgroundColor(Color.parseColor("#CC000000")); setPadding(12,6,12,6); setOnTouchListener(this)
        Handler(Looper.getMainLooper()).post(object : Runnable {
            override fun run() {
                val now = System.nanoTime(); frames++
                if (now - lastTime >= 1_000_000_000) {
                    fps = (frames * 1_000_000_000f / (now - lastTime)).toInt(); text = "FPS: $fps"; frames = 0; lastTime = now
                }
